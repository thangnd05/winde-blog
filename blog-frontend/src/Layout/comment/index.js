...
function Comment() {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const response = await axios.get(`https://api.winde.site/api/posts/${id}/comments`);
                const commentsWithUsers = await Promise.all(
                    response.data.map(async (comment) => {
                        try {
                            const userResponse = await axios.get(`https://api.winde.site/api/user/${comment.userId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Gửi bình luận mới
            const response = await axios.post("https://api.winde.site/api/comment", {
                content: comment,
                userId: userId,
            ...
            const userResponse = await axios.get(`https://api.winde.site/api/user/${userId}`);
            const newComment = {
                ...response.data,
    ...
    const deleteComment = (commentId) => {
        if (window.confirm("Bạn có chắc chắn muốn xóa bình luận này?")) {
            ...
            axios
                .delete(`https://api.winde.site/api/comment/${commentId}`)
                .then(() => {
    ...
    const handleSave = (commentId) => {
        ...
        axios
            .put(`https://api.winde.site/api/comment/${commentId}`, { content: newComment })
            .then(() => {
