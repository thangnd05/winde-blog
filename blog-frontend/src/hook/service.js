...
export const fetchUserId = async (setUserId) => {
    ...
    // try {
    //     const response = await axios.get(`https://api.winde.site/api/users?username=${currentUser.username}`);
    ...
    try {
        const response = await axios.get(`https://api.winde.site/api/mail/${currentUser.email}`);
        setUserId(response.data.user_id); // Lưu userId vào state
        // console.log("id", response.data.user_id);
    } catch (error) {
        console.error("Lỗi khi lấy dữ liệu người dùng:", error);
