...
function PostTabeProFile() {
  ...
  const handleDelete = (postId) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa bài viết này?")) {
      ...
      axios
        .delete(`https://api.winde.site/api/posts/${postId}`)
        .then(() => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://api.winde.site/api/posts/user/${userId}`);
        ...
        const postsWithDetails = await Promise.all(
          postsData.map(async (post) => {
            ...
            try {
              const categoryResponse = await axios.get(`https://api.winde.site/api/category/${post.categoryId}`);
            ...
            try {
              const userResponse = await axios.get(`https://api.winde.site/api/user/${post.userId}`);
